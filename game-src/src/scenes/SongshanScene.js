...
export class SongshanScene extends Phaser.Scene {
  ...
  createBoss() {
    ...
    this.bossLabel = this.add.text(4560, 46, '☯ 铁头僧王', {
  ...
  createBossHPBar() {
    ...
    this.bossHPName = this.add.text(cx, barY - 15, '☯ 铁头僧王  第一阶段', {
  ...
  lockBossArena() {
    ...
    const msg = this.add.text(4200, 96, '⚠️ 铁头僧王！退路已封！', {
  ...
  onBossPhaseChange(boss, phase) {
    if (phase === 2) {
      ...
      if (this.bossHPName) this.bossHPName.setText('☯ 铁头僧王  ⚡ 第二阶段').setStyle({ color: '#ff6600' });
      if (this.bossHPBG) this.bossHPBG.setStrokeStyle(2, 0xff6600, 1);
      this.showLevelBanner('⚡ 铁头僧王·怒气大发！');
      ...
      if (this.bossHPName) this.bossHPName.setText('☯ 铁头僧王  💀 狂怒').setStyle({ color: '#ff4444' });
      if (this.bossHPBG) this.bossHPBG.setStrokeStyle(2, 0xdd1100, 1);
      this.showLevelBanner('💀 铁头僧王·狂怒之力！极度危险！');
  ...
  onBossDefeated(boss) {
    ...
    this.showLevelBanner('☯ 铁头僧王已败！');
    this.time.delayedCall(1200, () => this.showVictoryDialogue());
  }

  showVictoryDialogue() {
    const lines = [
      '旁白：「铁头僧王一败涂地，少林禅武之力流转于身…」',
  ...
  finishLevel() {
    ...
    const text = this.add.text(GAME_WIDTH / 2, GAME_HEIGHT / 2, '🗺️ 嵩山通关！五岳征途继续！', {
  ...
  update() {
    ...
    if (this.boss && this.boss.active && !this.bossEncounterShown && this.player.x >= 4080) {
      ...
      this.showLevelBanner('☯ 铁头僧王登场！集中精力！');
      this.showBossHPBar();
      this.lockBossArena();
    }
    ...
    if (this.hud && this.hud.scene.isActive()) {
      ...
      if (this.quizUI && this.quizUI.isOpen) {
        ...
        const hint = ph === 3 ? '💀 狂怒！躲铁棍·X护体·C黄龙·J剑击'
                   : ph === 2 ? '⚡ 第二阶段！警惕冲锋·X护体·J剑击'
                   : '☯ 铁头僧王！J 剑击 · C 黄龙震地 · X 护体';
        ...
        this.hud.setHint(`${wLabel} · Q切换 | J剑/拳 | C黄龙 | V易筋经 | X护体`);
      } else {
