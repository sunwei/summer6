...
export class BootScene extends Phaser.Scene {
  ...
  create() {
    ...
    // 泰山花岗岩台阶（暖灰色，历史沧桑感）
    g.clear();
    rect(0xb0a898, 0, 0, 32, 6);   // 台阶顶面（暖灰）
    rect(0x989080, 0, 4, 32, 2);   // 缝隙
    rect(0x7a726a, 0, 6, 32, 26);  // 岩体
    rect(0x5a524a, 16, 6, 2, 26);  // 纵向裂纹
    rect(0xc8c0b8, 0, 0, 14, 2);   // 左高光
    rect(0xc8c0b8, 18, 0, 14, 2);  // 右高光
    g.generateTexture('tile_taishan', 32, 32);

    // 泰山天兵（敌人，红金天甲，32×48）
    g.clear();
    rect(0xd4a574, 10, 4, 12, 10);  // 面部
    rect(0xcc3300, 6, 0, 20, 6);    // 红色天甲头盔
    rect(0xffd700, 7, 3, 18, 3);    // 金色头盔边
    rect(0x3d1a00, 12, 8, 2, 2);    // 左眼
    rect(0x3d1a00, 18, 8, 2, 2);    // 右眼
    rect(0xd4a574, 14, 11, 4, 2);   // 嘴
    rect(0xcc3300, 6, 14, 20, 18);  // 红色天甲身
    rect(0xffd700, 8, 16, 16, 4);   // 金色胸纹
    rect(0xffd700, 6, 30, 20, 2);   // 金色腰带
    rect(0xcc3300, 2, 14, 4, 16);   // 左臂甲
    rect(0xcc3300, 26, 14, 4, 16);  // 右臂甲
    rect(0xffd700, 28, 8, 2, 14);   // 持戟（金色）
    rect(0xcc3300, 6, 32, 8, 12);   // 左腿甲
    rect(0xcc3300, 18, 32, 8, 12);  // 右腿甲
    rect(0x8b1010, 4, 42, 10, 4);   // 左战靴
    rect(0x8b1010, 18, 42, 10, 4);  // 右战靴
    g.generateTexture('taishan_enemy', 32, 48);

    // 天煞将（BOSS，金甲红袍神将，威武霸气，32×48）
    g.clear();
    rect(0xd4a574, 8, 4, 16, 12);   // 面部
    rect(0xcc3300, 4, 0, 24, 6);    // 威严头盔（深红）
    rect(0xffd700, 5, 1, 22, 4);    // 金色盔顶
    rect(0xff8800, 3, 3, 26, 4);    // 橙色盔带
    rect(0x3d1a00, 10, 9, 3, 2);    // 左眼（锐利）
    rect(0x3d1a00, 19, 9, 3, 2);    // 右眼
    rect(0x8b0000, 12, 13, 8, 2);   // 怒口
    rect(0xcc3300, 4, 16, 24, 18);  // 红色神将袍
    rect(0xffd700, 6, 18, 20, 4);   // 金色胸甲
    rect(0xffd700, 4, 32, 24, 2);   // 金色腰带
    rect(0xcc3300, 0, 16, 4, 16);   // 左臂
    rect(0xcc3300, 28, 16, 4, 16);  // 右臂
    rect(0xd4a574, 0, 26, 4, 6);    // 左拳
    rect(0xd4a574, 28, 26, 4, 6);   // 右拳
    rect(0xcc3300, 6, 34, 8, 14);   // 左腿
    rect(0xcc3300, 18, 34, 8, 14);  // 右腿
    // 神将天戟（长兵器）
    rect(0xffd700, 30, 2, 3, 30);   // 戟柄（金色）
    rect(0xff8800, 28, 0, 6, 4);    // 戟头
    rect(0xffd700, 29, 30, 3, 3);   // 戟底
    g.generateTexture('taishan_boss', 32, 48);

    // 泰山智者（白发白袍老先生，32×64）
    g.clear();
    rect(0xd4a574, 10, 8, 12, 10);  // 面部
    rect(0xf8f8f0, 8, 3, 16, 7);    // 白发
    rect(0xffffff, 7, 4, 18, 3);    // 发顶高光
    rect(0x3d1a00, 12, 12, 2, 1);   // 左眼（慈祥）
    rect(0x3d1a00, 18, 12, 2, 1);   // 右眼
    rect(0xf8f8f0, 10, 15, 12, 6);  // 白须（长）
    rect(0xf0efe8, 8, 20, 16, 16);  // 白色长袍上身
    rect(0xe8e7e0, 6, 22, 20, 4);   // 袍面暗纹
    rect(0xf0efe8, 4, 20, 4, 14);   // 左袖
    rect(0xf0efe8, 24, 20, 4, 14);  // 右袖
    rect(0xf0efe8, 8, 36, 16, 14);  // 下摆
    rect(0x4a3000, 6, 50, 10, 6);   // 左鞋
    rect(0x4a3000, 16, 50, 10, 6);  // 右鞋
    rect(0x7a6030, 3, 12, 3, 42);   // 竹杖
    rect(0x9a8040, 1, 10, 5, 4);    // 杖顶饰
    g.generateTexture('npc_sage', 32, 64);

    // 泰山缆车（象形图标，48×32）
    g.clear();
    // 缆绳
    rect(0x5a4a38, 0, 4, 48, 2);
    // 车厢
    rect(0x4488cc, 14, 6, 22, 18);  // 蓝色车厢
    rect(0x66aadd, 16, 8, 18, 8);   // 车厢高光
    rect(0x2266aa, 14, 22, 22, 2);  // 车厢底部
    // 窗户
    rect(0xddf0ff, 16, 10, 6, 5);
    rect(0xddf0ff, 26, 10, 6, 5);
    // 滑轮
    rect(0x887766, 22, 2, 6, 6);
    rect(0xaaa090, 23, 3, 4, 4);
    g.generateTexture('cable_car', 48, 32);

    // 泰山无字碑（标志性景点，32×60）
    g.clear();
    rect(0xa09880, 6, 4, 20, 52);   // 碑体（暖灰石色）
    rect(0xbcb4a0, 8, 2, 16, 4);    // 碑顶弧形
    rect(0x8a8070, 4, 54, 24, 6);   // 碑座
    rect(0x706858, 6, 4, 4, 52);    // 左侧阴影
    rect(0xd0c8b0, 14, 6, 8, 2);    // 顶部高光
    // 碑面风化纹理
    rect(0x8a8070, 10, 12, 12, 1);
    rect(0x8a8070, 8, 20, 16, 1);
    rect(0x8a8070, 10, 28, 14, 1);
    rect(0x8a8070, 9, 36, 12, 1);
    g.generateTexture('stele_wuzi', 32, 60);  // 无字碑

    // 泰山煎饼（山东特产，28×28）
    g.clear();
    rect(0xd4a060, 2, 8, 24, 14);   // 煎饼主体（金黄色）
    rect(0xe8b870, 4, 6, 20, 8);    // 顶面（更亮）
    rect(0xb88040, 2, 18, 24, 4);   // 底部阴影
    rect(0x6a3010, 6, 10, 16, 2);   // 烙印纹理
    rect(0x6a3010, 8, 14, 12, 1);   // 纹理2
    rect(0x228b22, 8, 7, 4, 2);     // 葱花绿
    g.generateTexture('food_jianbing', 28, 28);

    // 泰山豆腐（特色，28×28，嫩白带紫）
    g.clear();
    rect(0xfffff2, 2, 4, 24, 18);
    rect(0xffffff, 4, 4, 20, 5);
    rect(0xe8e8de, 2, 20, 24, 4);
    rect(0xffffee, 6, 8, 16, 10);
    rect(0x993388, 9, 9, 3, 3);     // 紫色（泰山特色豆腐）
    rect(0x228b22, 14, 10, 3, 2);   // 葱花
    g.generateTexture('food_taishan_tofu', 28, 28);

    // 青龙晶（东岳气运碎片，蓝绿色，24×24）
    g.clear();
    rect(0x66ddaa, 10, 0, 4, 4);
    rect(0x44cc88, 6, 4, 12, 4);
    rect(0x22aa66, 2, 8, 20, 8);
    rect(0x44cc88, 6, 16, 12, 4);
    rect(0x66ddaa, 10, 20, 4, 4);
    rect(0x00dd88, 4, 10, 16, 2);   // 青龙纹
    rect(0xaaffcc, 12, 2, 2, 2);    // 高光
    g.generateTexture('crystal_qinglong_item', 24, 24);
