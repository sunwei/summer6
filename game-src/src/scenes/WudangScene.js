// This file is the first full playable level: jump, fight, talk to the master, unlock Taiji, and reach the portal.
...
export class WudangScene extends Phaser.Scene {
  ...
  create() {
    ...
    this._musicTheme = 'wudang'; // track current music state
    ...
    // The physics world is taller than the camera view, so falling into the cliff really feels dangerous.
    this.physics.world.setBounds(0, 0, 3200, 520);
    // Reserve bottom 80px for the HUD hotbar — game world only uses top 460px
    const PLAY_H = GAME_HEIGHT - 80;
    this.cameras.main.setViewport(0, 0, GAME_WIDTH, PLAY_H);
    this.cameras.main.setBounds(0, 0, 3200, PLAY_H);
  ...
  createBackground() {
    ...
    // Small pagoda shapes add a Wudang feeling to the mid background.
    ...
    for (let x = -50; x < 3300; x += 70) {
      near.fillTriangle(x, 540, x + 26, 430 - (x % 3) * 20, x + 52, 540);
      near.fillTriangle(x + 20, 540, x + 48, 410 - (x % 4) * 18, x + 76, 540);
    }

    for (let i = 0; i < 7; i += 1) {
      const cloud = this.add.container(Phaser.Math.Between(-100, 3200), Phaser.Math.Between(60, 180), [
  ...
  createPlatforms() {
    ...
    // Ground with a jump gap in the middle.
  ...
  createNpcAndPortal() {
    const dialogues = [
      '真人：「小北，老夫等候多时。」',
      '真人：「蛟龙夺你气运，你需习得太极，方能克之。」',
      '真人：「太极之道，以柔克刚。按住 K 键，化解敌人攻击。」',
      '真人：「去吧，五岳之路，老夫祝你平安。」',
      '✅ 习得太极拳！热键栏第6格已解锁。',
    ];
    ...
    this.npc = new NPC(this, 2900, 400, 'npc_taoist', dialogues, () => {
      if (skillSystem.collect(ITEMS.SKILL_TAIJI)) {
        this.showLevelBanner('✨ 太极拳 已解锁！');
        this.activatePortal(true);
      }
    });

    this.portalGlow = this.add.ellipse(3100, 360, 40, 76, COLORS.TEAL, 0.18).setVisible(false);
    this.portal = this.physics.add.staticImage(3100, 360, 'portal_frame');
  ...
  createQuizItems() {
    ...
    positions.forEach(([x, y], i) => {
      ...
      const hint = this.add.text(x, y - 30, '📜 答题', {
  ...
  showControlsReminder() {
    const reminder = this.add
      .text(GAME_WIDTH / 2, 76, '← → 移动  |  ↑/空格 跳跃  |  J 攻击  |  Z 烈焰  |  F 交谈', {
        fontSize: '20px',
        color: '#ffffff',
  ...
  activatePortal(showBanner) {
    ...
    if (showBanner) {
      this.showLevelBanner('✨ 传送门已开启！');
    }
  }

  showLevelBanner(text) {
    const bg = this.add.rectangle(GAME_WIDTH / 2, 112, 320, 38, 0x0d1117, 0.92).setScrollFactor(0).setDepth(950);
    bg.setStrokeStyle(2, COLORS.GOLD, 1);

    const label = this.add
      .text(GAME_WIDTH / 2, 112, text, {
        fontSize: '24px',
        color: '#fff3b0',
        fontStyle: 'bold',
      })
  ...
  handlePlayerEnemyOverlap(player, enemy) {
    if (!enemy.active || this.levelFinished) {
      return;
    }

    if (player.isGuarding()) {
      enemy.patrolDirection *= -1;
      enemy.setVelocityX(enemy.patrolDirection * 110);
      this.burstParticles(enemy.x, enemy.y, 8, 0xfff4b0);
      return;
    }

    player.hurt(10, enemy.x);
  }
  ...
  damageEnemy(enemy, damage = 10) {
    ...
    enemy.hp = typeof enemy.hp === 'number' ? enemy.hp : 10;
    enemy.hp -= damage;

    if (enemy.hp > 0) {
      sfx.play('enemy_hit');
      this.burstParticles(enemy.x, enemy.y, 6, 0xffd27a);
      return;
    }
  ...
  /**
   * 朱雀烈焰 explosion — visual radius matches the actual hit range exactly.
   * Three layered effects: expanding shockwave ring (hits edge of range), inner glow,
   * and scattered fire particles flung throughout the whole range area.
   */
  zhuqueBlast(x, y, range) {
    // ── Outer shockwave ring: expands to exactly `range` px radius ──
    ...
    // ── Inner fire ball: expands to ~55% of range ──
    ...
    // ── Fire particles scattered throughout the full range ──
    const particleCount = Math.round(16 + (range / 150) * 12); // more particles at higher wisdom
    for (let i = 0; i < particleCount; i++) {
      const bit = this.add.image(x, y, 'particle')
        .setTint(i < particleCount * 0.6 ? 0xff4400 : 0xffaa00)
        .setAlpha(0.95)
        .setDepth(50);
      const angle = Phaser.Math.FloatBetween(0, Math.PI * 2);
      // Distribute across [20px … range] so the whole AOE area is visually filled
  ...
  finishLevel() {
    ...
    bus.emit(EVENTS.LEVEL_COMPLETE, { next: '嵩山 · 少林寺' });
    ...
    const text = this.add
      .text(GAME_WIDTH / 2, GAME_HEIGHT / 2, '下一关：嵩山 · 少林寺', {
        fontSize: '36px',
        color: '#fff1a6',
        fontStyle: 'bold',
        stroke: '#000000',
  ...
  update() {
    ...
    this.enemies.getChildren().forEach((enemy) => {
      ...
      enemy.setVelocityX(enemy.patrolDirection * 55);
    ...
    // Dynamic music: switch to battle if enemy is close, back to calm when clear
    if (!this.levelFinished) {
    ...
    if (this.hud && this.hud.scene.isActive()) {
      ...
      if (this.quizUI && this.quizUI.isOpen) {
      ...
      } else if (this.portalActive && Math.abs(this.player.x - this.portal.x) < 80) {
        this.hud.setHint('走进传送门，前往下一关');
        ...
        this.hud.setHint('J 出拳八卦弹  |  K 太极护体');
      } else {
